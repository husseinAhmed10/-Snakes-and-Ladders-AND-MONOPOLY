...
int Input::GetInteger(Output *pO) const 
{
	...
	string x;
	pO->PrintMessage("Please enter an integer: ");		//msh mawgood 3nd Ammar
	x = GetSrting(pO);
	return stoi(x);
...
{	
	...
	if ( UI.InterfaceMode == MODE_DESIGN )	
	{
		...
		{	
			...
			case ITM_Delete_Game_Object: return DELETE_GAME_OPJECT;
	...
	else	
	{
		...
		if (y >= 0 && y < UI.ToolBarHeight)
		{
			...
			case ITM_EXIT_FROM_PLAY_MODE: return EXIT;
