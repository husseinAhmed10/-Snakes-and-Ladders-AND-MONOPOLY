...
void AddCardAction::ReadActionParameters()
{
	...
	if (cardPosition.GetCellNum() < 1 || cardPosition.GetCellNum() > 99)	//I think mlhash lazma
	{
		pOut->PrintMessage("Invalid cell position");
	}

	// 4- Read the "cardPosition" parameter (its cell position) and set its data member
	pOut->PrintMessage("Please select the cell position:");
...
void AddCardAction::Execute()
{
	...
	case 11:
		...
		break;
	case 12:
		pCard = new CardTweleve(cardPosition);
		break;
	case 13:
	...
	if (pCard)
	{
		...
		if (added == false)
		{
			pGrid->PrintErrorMessage("GameObject cannot be added in the Cell");
		}
	}
